...
Date: 21 Jan 2021
Objective: Output a LaTeX table similar to Table 8 in the paper
*/

// I mostly recycled the table 3=6 script
...
global interest2 "treat2016  yr_2016"
global store_results "treat2016 treatment_class yr_2016 _cons"
global store_results2 "treat2016 yr_2016 _cons"
global controls "female_prof instate freshman american ACumGPA small_class"
global controlsfe "instate freshman american ACumGPA"

// Get the data for the table
forvalues i = 1/4 {
    // LPM 1 no controls Took in a year
    local var : word `i' of $store_results
    reg gradePrinciples $interest if female==1, cluster(class_fe2)
	...
	// LPM 2  controls Took in a year
    reg gradePrinciples $interest $controls if female==1, cluster(class_fe2)
    global lpm2_`var': di %6.3f e(b)[1,`i']
	if `i' == 4 {
		global lpm2_`var': di %6.3f e(b)[1,10]
	}
	global lpm2_n = e(N)
	boottest `var', reps(1000) weight(webb) bootcluster(class_fe2) nograph
...
// Get the data for the table
forvalues i = 1/3 {
    local var : word `i' of $store_results2
	// LPM 3 no controls Took econ
    reg gradePrinciples $interest2 i.class_fe if female==1, cluster(class_fe2)
	...
	// LPM 4 controls Took econ
    reg gradePrinciples $interest2 $controlsfe i.class_fe if female==1, cluster(class_fe2)
    global lpm4_`var': di %6.3f e(b)[1,`i']
	if `i' == 3 {
		global lpm4_`var': di %6.3f e(b)[1,16]
	}
	global lpm4_n = e(N)
	boottest `var', reps(1000) weight(webb) bootcluster(class_fe2) nograph
...
texdoc init "figures_tables\table8.tex", replace force
tex \begin{tabular}{lcccc} \toprule 
tex  & Grade Principles & Grade Principles & Grade Principles & Grade Principles \tabularnewline \midrule
tex  &  &  &  & \tabularnewline
...
tex Treatment class (in 2015) & ${lpm1_treatment_class} & ${lpm2_treatment_class} &   &  \tabularnewline
tex  & (${lpm1_p_treatment_class}) & (${lpm2_p_treatment_class}) &   &  \tabularnewline
tex  & {[}${lpm1_c1_treatment_class},${lpm1_c2_treatment_class}{]} & {[}${lpm2_c1_treatment_class},${lpm2_c2_treatment_class}{]} &     &  \tabularnewline
...
tex Class fixed effects & No & No & Yes & Yes\tabularnewline
tex  &  &  &  & \tabularnewline
