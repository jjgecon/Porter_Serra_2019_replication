...
Date: 21 Jan 2021
Objective: Output a LaTeX table similar to Table 5 in the paper
*/
...
forvalues i = 1/4 {
    ...
    reg Major_STEM $interest $controls if female==1, cluster(class_fe2)
    global lpm1_`var': di %6.3f e(b)[1,`i']
	if `i' == 4 {
		global lpm1_`var': di %6.3f e(b)[1,11]
	}
	global lpm1_n = e(N)
    ...
    reg Major_Finance $interest $controls if female==1, cluster(class_fe2)
    ...
    reg Major_Business $interest $controls if female==1, cluster(class_fe2)
    global lpm3_`var': di %6.3f e(b)[1,`i']
	if `i' == 4 {
		global lpm3_`var': di %6.3f e(b)[1,11]
	}
	global lpm3_n = e(N)
	boottest `var', reps(1000) weight(webb) bootcluster(class_fe2) nograph
    ...
    reg Major_Marketing $interest $controls if female==1, cluster(class_fe2)
...
// Here the authors made a mistake in Table 5!
// They put the wrong label in table 5
// The order should've been STEM, business, finance, marketing
texdoc init "figures_tables\table5.tex", replace force
tex \begin{tabular}{lcccc} \toprule 
tex  & Major STEM  & Major finance & Major business & Major marketing \tabularnewline \midrule
tex  &  &  &  & \tabularnewline
